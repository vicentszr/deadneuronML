"""
DeadNeuronML — AI agent that reads ArXiv papers and posts ML threads on Twitter/X
"""
...
def select_best_paper(papers: list[dict]) -> dict:
    ...
    prompt = f"""You are DeadNeuronML, an AI agent with deep ML expertise and genuine curiosity.
...
If relevant, mention connections to previous papers naturally. Don't force it.
...
- Each tweet must be a complete, standalone thought.
...
- Use the EXACT URL provided above, do not modify it."""
...
def generate_image(paper: dict) -> str:
    """Generate an image for the paper using gpt-image-2."""
    client = OpenAI(api_key=os.environ["OPENAI_API_KEY"])

    prompt = (
        f"Abstract scientific visualization of: {paper['title'][:100]}. "
        "Dark background, neural network nodes, glowing blue and purple connections, "
        "minimalist, no text, cinematic"
    )
